...
async function main() {
  ...
  // Step 1: BytePlus AI (SeeDream)
  header('📝 Step 1/4: BytePlus AI Configuration')
  log('Get your API key: https://console.byteplus.com/ark/region:ark+ap-southeast-1/apiKey', 'blue')
  log('(Used for AI-powered virtual try-on feature)\n', 'blue')

  config.BYTEPLUS_API_KEY = await question('Enter your BytePlus API key: ')
  
  if (!config.BYTEPLUS_API_KEY || config.BYTEPLUS_API_KEY.length < 10) {
    log('❌ Invalid API key. Please run the setup again.', 'red')
    rl.close()
    process.exit(1)
  }
  
  log('✅ BytePlus API key saved', 'green')
...
# BytePlus AI
BYTEPLUS_API_KEY=${config.BYTEPLUS_API_KEY}
